...
// refresh gauge level
JustGage.prototype.refresh = function (value, title, showValueAsMin) {
  ...
  var color = getColorForPercentage((value - this.config.min) / (this.config.max - this.config.min), this.config.levelColors, this.config.levelColorsGradient);

  if (title) {
    this.canvas.getById(this.config.id+"-txtvalue").attr({"text":title});
  }

  if (showValueAsMin) {
    this.canvas.getById(this.config.id+"-txtmin").attr({"text":value});
  }

  this.canvas.getById(this.config.id+"-level").animate({pki: [value, this.config.min, this.config.max, this.params.widgetW, this.params.widgetH,  this.params.dx, this.params.dy, this.config.gaugeWidthScale], "fill":color},  this.config.refreshAnimationTime, this.config.refreshAnimationType);
};
